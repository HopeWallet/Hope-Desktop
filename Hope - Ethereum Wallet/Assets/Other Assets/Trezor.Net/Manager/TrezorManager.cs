...
namespace Trezor.Net
{
    public class TrezorManager : TrezorManagerBase<MessageType>
    {
        ...
        protected override async Task<object> PinMatrixAckAsync(string pin)
        {
            ...
            if (retVal is Failure)
            {
                throw new FailureException<Failure>("PIN Attempt Failed.", retVal as Failure);
            }
        ...
        protected override async Task<object> ButtonAckAsync()
        {
            ...
            if (retVal is Failure)
            {
                throw new FailureException<Failure>("USer didn't push the button.", retVal as Failure);
            }
        ...
        {
            if (returnMessage is Failure)
            {
                var failure = returnMessage as Failure;
        ...
        protected override object GetEnumValue(string messageTypeString)
        {
            ...
            var isValid = Enum.TryParse(messageTypeString, out messageType);
            if (!isValid)
            {
